...
type ImageQuery struct {
	...
	withCompressed    *FileQuery
	withOriginal      *FileQuery
	withThumbnail320x *FileQuery
	withThumbnail768x *FileQuery
	withUser          *UserQuery
...
// QueryCompressed chains the current query on the "compressed" edge.
func (iq *ImageQuery) QueryCompressed() *FileQuery {
...
func (iq *ImageQuery) QueryOriginal() *FileQuery {
	query := (&FileClient{config: iq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryThumbnail320x chains the current query on the "thumbnail320x" edge.
func (iq *ImageQuery) QueryThumbnail320x() *FileQuery {
	query := (&FileClient{config: iq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryThumbnail768x chains the current query on the "thumbnail768x" edge.
func (iq *ImageQuery) QueryThumbnail768x() *FileQuery {
	query := (&FileClient{config: iq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (iq *ImageQuery) QueryMomentImages() *MomentImageQuery {
	query := (&MomentImageClient{config: iq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (iq *ImageQuery) Clone() *ImageQuery {
	...
	return &ImageQuery{
		...
		withCompressed:    iq.withCompressed.Clone(),
		withOriginal:      iq.withOriginal.Clone(),
		withThumbnail320x: iq.withThumbnail320x.Clone(),
		withThumbnail768x: iq.withThumbnail768x.Clone(),
		withUser:          iq.withUser.Clone(),
		withMomentImages:  iq.withMomentImages.Clone(),
...
// WithCompressed tells the query-builder to eager-load the nodes that are connected to
// the "compressed" edge. The optional arguments are used to configure the query builder of the edge.
func (iq *ImageQuery) WithCompressed(opts ...func(*FileQuery)) *ImageQuery {
	query := (&FileClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withCompressed = query
	return iq
}
...
func (iq *ImageQuery) WithOriginal(opts ...func(*FileQuery)) *ImageQuery {
	query := (&FileClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withOriginal = query
	return iq
}

// WithThumbnail320x tells the query-builder to eager-load the nodes that are connected to
// the "thumbnail320x" edge. The optional arguments are used to configure the query builder of the edge.
func (iq *ImageQuery) WithThumbnail320x(opts ...func(*FileQuery)) *ImageQuery {
	query := (&FileClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withThumbnail320x = query
	return iq
}

// WithThumbnail768x tells the query-builder to eager-load the nodes that are connected to
// the "thumbnail768x" edge. The optional arguments are used to configure the query builder of the edge.
func (iq *ImageQuery) WithThumbnail768x(opts ...func(*FileQuery)) *ImageQuery {
	query := (&FileClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withThumbnail768x = query
	return iq
}
...
func (iq *ImageQuery) WithMomentImages(opts ...func(*MomentImageQuery)) *ImageQuery {
	query := (&MomentImageClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withMomentImages = query
	return iq
}
...
func (iq *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	var (
		...
		_spec       = iq.querySpec()
		loadedTypes = [6]bool{
			iq.withCompressed != nil,
			iq.withOriginal != nil,
			iq.withThumbnail320x != nil,
			iq.withThumbnail768x != nil,
			iq.withUser != nil,
			iq.withMomentImages != nil,
		}
	)
	...
	if query := iq.withCompressed; query != nil {
		if err := iq.loadCompressed(ctx, query, nodes, nil,
			func(n *Image, e *File) { n.Edges.Compressed = e }); err != nil {
			return nil, err
		}
	}
	if query := iq.withOriginal; query != nil {
		if err := iq.loadOriginal(ctx, query, nodes, nil,
			func(n *Image, e *File) { n.Edges.Original = e }); err != nil {
			return nil, err
		}
	}
	if query := iq.withThumbnail320x; query != nil {
		if err := iq.loadThumbnail320x(ctx, query, nodes, nil,
			func(n *Image, e *File) { n.Edges.Thumbnail320x = e }); err != nil {
			return nil, err
		}
	}
	if query := iq.withThumbnail768x; query != nil {
		if err := iq.loadThumbnail768x(ctx, query, nodes, nil,
			func(n *Image, e *File) { n.Edges.Thumbnail768x = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := iq.withMomentImages; query != nil {
		if err := iq.loadMomentImages(ctx, query, nodes,
			func(n *Image) { n.Edges.MomentImages = []*MomentImage{} },
			func(n *Image, e *MomentImage) { n.Edges.MomentImages = append(n.Edges.MomentImages, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ImageQuery) loadCompressed(ctx context.Context, query *FileQuery, nodes []*Image, init func(*Image), assign func(*Image, *File)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Image)
	for i := range nodes {
		fk := nodes[i].CompressedID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "compressed_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (iq *ImageQuery) loadOriginal(ctx context.Context, query *FileQuery, nodes []*Image, init func(*Image), assign func(*Image, *File)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Image)
	for i := range nodes {
		fk := nodes[i].OriginalID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "original_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (iq *ImageQuery) loadThumbnail320x(ctx context.Context, query *FileQuery, nodes []*Image, init func(*Image), assign func(*Image, *File)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Image)
	for i := range nodes {
		fk := nodes[i].Thumbnail320xID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "thumbnail_320x_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (iq *ImageQuery) loadThumbnail768x(ctx context.Context, query *FileQuery, nodes []*Image, init func(*Image), assign func(*Image, *File)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Image)
	for i := range nodes {
		fk := nodes[i].Thumbnail768xID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "thumbnail_768x_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (iq *ImageQuery) loadMomentImages(ctx context.Context, query *MomentImageQuery, nodes []*Image, init func(*Image), assign func(*Image, *MomentImage)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(momentimage.FieldImageId)
	}
	query.Where(predicate.MomentImage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(image.MomentImagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "imageId" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (iq *ImageQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := iq.ctx.Fields; len(fields) > 0 {
		...
		if iq.withCompressed != nil {
			_spec.Node.AddColumnOnce(image.FieldCompressedID)
		}
		if iq.withOriginal != nil {
			_spec.Node.AddColumnOnce(image.FieldOriginalID)
		}
		if iq.withThumbnail320x != nil {
			_spec.Node.AddColumnOnce(image.FieldThumbnail320xID)
		}
		if iq.withThumbnail768x != nil {
			_spec.Node.AddColumnOnce(image.FieldThumbnail768xID)
		}
		if iq.withUser != nil {
			_spec.Node.AddColumnOnce(image.FieldUploadedBy)
		}
	}
