...
func (vq *VideoQuery) QueryMomentVideos() *MomentVideoQuery {
	query := (&MomentVideoClient{config: vq.config}).Query()
...
func (vq *VideoQuery) QueryFile() *FileQuery {
	query := (&FileClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VideoQuery) QueryPoster() *FileQuery {
	query := (&FileClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (vq *VideoQuery) Clone() *VideoQuery {
	...
	return &VideoQuery{
		...
		withMomentVideos: vq.withMomentVideos.Clone(),
		withFile:         vq.withFile.Clone(),
		withPoster:       vq.withPoster.Clone(),
...
func (vq *VideoQuery) WithMomentVideos(opts ...func(*MomentVideoQuery)) *VideoQuery {
	query := (&MomentVideoClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withMomentVideos = query
	return vq
}
...
func (vq *VideoQuery) WithFile(opts ...func(*FileQuery)) *VideoQuery {
	query := (&FileClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (vq *VideoQuery) WithPoster(opts ...func(*FileQuery)) *VideoQuery {
	query := (&FileClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (vq *VideoQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Video, error) {
	...
	if query := vq.withMomentVideos; query != nil {
		if err := vq.loadMomentVideos(ctx, query, nodes,
			func(n *Video) { n.Edges.MomentVideos = []*MomentVideo{} },
			func(n *Video, e *MomentVideo) { n.Edges.MomentVideos = append(n.Edges.MomentVideos, e) }); err != nil {
			return nil, err
		}
	}
	if query := vq.withFile; query != nil {
		if err := vq.loadFile(ctx, query, nodes, nil,
			func(n *Video, e *File) { n.Edges.File = e }); err != nil {
			return nil, err
		}
	}
	if query := vq.withPoster; query != nil {
		if err := vq.loadPoster(ctx, query, nodes, nil,
			func(n *Video, e *File) { n.Edges.Poster = e }); err != nil {
			return nil, err
		}
	}
	if query := vq.withUser; query != nil {
		if err := vq.loadUser(ctx, query, nodes, nil,
			func(n *Video, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VideoQuery) loadMomentVideos(ctx context.Context, query *MomentVideoQuery, nodes []*Video, init func(*Video), assign func(*Video, *MomentVideo)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(momentvideo.FieldVideoId)
	}
	query.Where(predicate.MomentVideo(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(video.MomentVideosColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "videoId" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (vq *VideoQuery) loadFile(ctx context.Context, query *FileQuery, nodes []*Video, init func(*Video), assign func(*Video, *File)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "fileId" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (vq *VideoQuery) loadPoster(ctx context.Context, query *FileQuery, nodes []*Video, init func(*Video), assign func(*Video, *File)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Video)
	for i := range nodes {
		fk := nodes[i].PosterId
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "posterId" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (vq *VideoQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Video, init func(*Video), assign func(*Video, *User)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Video)
	for i := range nodes {
...
func (vq *VideoQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := vq.ctx.Fields; len(fields) > 0 {
		...
		if vq.withFile != nil {
			_spec.Node.AddColumnOnce(video.FieldFileId)
		}
		if vq.withPoster != nil {
			_spec.Node.AddColumnOnce(video.FieldPosterId)
		}
		if vq.withUser != nil {
			_spec.Node.AddColumnOnce(video.FieldUploadedBy)
		}
	}
